...
const labels = { approved: 'Approved', conditional: 'Conditional', rejected: 'Rejected', no_answer: 'Unconfirmed' };
...
export function render(el) {
  ...
  }).join('')}</tbody></table></div><div class="record-library">${rows.map(row => `<details class="record-detail"><summary><span>${escape(row.change_id)}</span> Read transcript &amp; evidence <span aria-hidden="true">↗</span></summary><p class="record-meta">${row.space === 'demo' ? 'Demo Studio rehearsal' : 'Workspace decision'} · ${escape(row.channel)} · Proposal ${escape(row.proposal_id || row.change_id)}</p>${row.confirmation_quote ? `<p class="confirmation-proof"><strong>Final human confirmation:</strong> “${escape(row.confirmation_quote)}”</p>` : ''}${row.condition_text ? `<p><strong>Condition:</strong> ${escape(row.condition_text)}</p>` : ''}${row.rationale_quote ? `<p><strong>Rationale:</strong> ${escape(row.rationale_quote)}</p>` : ''}${row.error ? `<p class="notice">${escape(row.error)}</p>` : ''}${row.diagnostics ? `<p class="record-meta">Voice service: ${escape(row.diagnostics.provider_decision || 'Not available')} · Interrupted agent turns: ${escape(row.diagnostics.interrupted_agent_turns ?? 0)} · Final confirmation: ${row.diagnostics.confirmation_verified ? 'Verified' : 'Not verified'}</p>` : ''}<div class="saved-transcript">${(row.transcript || []).map(line => `<p><strong>${line.role === 'agent' ? 'Hango' : escape(row.approver)}:</strong> ${escape(line.text)}</p>`).join('') || '<p>No transcript recorded.</p>'}</div><p class="record-meta">Conversation: ${escape(row.conversation_id || 'Not recorded')}<br>SHA-256: ${escape(row.diff_sha256 || row.diff_hash)}<br>Tests: ${row.test_evidence_matched ? 'Matched to this exact diff' : 'Not verified'}</p><details><summary>Exact diff reviewed</summary><pre class="diff">${escape(row.diff_snapshot || 'No diff recorded.')}</pre></details></details>`).join('')}</div>`;
}
...
export function printView() {
  ...
  popup.opener = null;
  popup.document.write(`<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Hango change ledger</title><style>
    body{font:14px/1.6 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#252525;max-width:850px;margin:45px auto;padding:0 25px}h1{font-size:30px;letter-spacing:-1px}h2{margin:0;font-size:20px}header{border-bottom:2px solid #222;margin-bottom:30px}section{break-inside:avoid;border-bottom:1px solid #bbb;padding:0 0 30px;margin:30px 0}.stamp{float:right;color:#c83232;border:4px double #c83232;padding:8px;font:26px serif;transform:rotate(-5deg)}dt{font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#666;margin-top:15px}dd{margin:3px 0;overflow-wrap:anywhere}pre{white-space:pre-wrap;overflow-wrap:anywhere;font:11px/1.6 monospace;background:#f4f4f4;padding:12px}.simulated{color:#9e6000;font-weight:700}button{padding:10px 18px;background:#222;color:#fff;border:0;cursor:pointer}.transcript p{margin:8px 0}@media print{button{display:none}body{margin:0}section{break-inside:auto}.stamp{print-color-adjust:exact}}
    </style></head><body><header><h1>Hango change ledger</h1><p>Spoken judgment, bound to the exact diff. Local demonstration record.</p></header><button onclick="window.print()">Print ledger</button>${rows.length ? rows.map(row => {
      ...
      return `<section><div class="stamp">${stamp}</div><h2>${escape(row.change_id)}</h2>${row.channel === 'canned' ? '<p class="simulated">SIMULATED APPROVAL · NOT A HUMAN SIGN-OFF</p>' : ''}<dl>${Object.entries(fields).map(([key, value]) => `<dt>${escape(key)}</dt><dd>${escape(value)}</dd>`).join('')}</dl><h3>Transcript</h3><div class="transcript">${(row.transcript || []).map(line => `<p><strong>${line.role === 'agent' ? 'Hango' : escape(row.approver)}:</strong> ${escape(line.text)}</p>`).join('') || '<p>No transcript recorded.</p>'}</div><h3>Exact diff reviewed</h3><pre>${escape(row.diff_snapshot || 'No diff was available.')}</pre></section>`;
