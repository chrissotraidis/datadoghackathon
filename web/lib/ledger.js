...
export function render(el) {
  ...
  el.innerHTML = `${storageWarning ? `<p class="notice">${escape(storageWarning)}</p>` : ''}<div class="ledger-scroll"><table class="ledger-table"><thead><tr><th scope="col">Change</th><th scope="col">Decision</th><th scope="col">Condition · verbatim</th><th scope="col">Approver</th><th scope="col">When</th><th scope="col">Channel</th><th scope="col">Diff</th></tr></thead><tbody>${rows.map(row => {
    ...
    return `<tr><td class="mono">${escape(row.change_id)}</td><td><span class="pill ${decision}">${labels[decision]}</span><span class="row-note">${escape(row.eligibility || (decision === 'approved' ? 'Review recorded' : 'HELD'))}</span></td><td>${row.condition_text ? `“${escape(row.condition_text)}”` : '<span class="muted">—</span>'}${row.condition_task ? '<span class="row-note">Open condition task</span>' : ''}</td><td>${escape(row.approver)}</td><td class="row-time">${escape(formatDate(row.approved_at))}</td><td>${row.channel === 'canned' ? '<span class="simulated">Simulated</span>' : escape(row.channel)}</td><td class="mono" title="${escape(row.diff_sha256 || row.diff_hash)}">${escape(row.diff_hash)}</td></tr>`;
  }).join('')}</tbody></table></div><div class="record-library">${rows.map(row => `<details class="record-detail"><summary><span>${escape(row.change_id)}</span> Read transcript &amp; evidence <span aria-hidden="true">↗</span></summary><p class="record-meta">${row.space === 'demo' ? 'Demo Studio rehearsal' : 'Workspace decision'} · ${escape(row.channel)} · Proposal ${escape(row.proposal_id || row.change_id)}</p>${row.condition_text ? `<p><strong>Condition:</strong> ${escape(row.condition_text)}</p>` : ''}${row.rationale_quote ? `<p><strong>Rationale:</strong> ${escape(row.rationale_quote)}</p>` : ''}<div class="saved-transcript">${(row.transcript || []).map(line => `<p><strong>${line.role === 'agent' ? 'Hanko' : escape(row.approver)}:</strong> ${escape(line.text)}</p>`).join('') || '<p>No transcript recorded.</p>'}</div><p class="record-meta">Conversation: ${escape(row.conversation_id || 'Not recorded')}<br>SHA-256: ${escape(row.diff_sha256 || row.diff_hash)}<br>Tests: ${row.test_evidence_matched ? 'Matched to this exact diff' : 'Not verified'}</p><details><summary>Exact diff reviewed</summary><pre class="diff">${escape(row.diff_snapshot || 'No diff recorded.')}</pre></details></details>`).join('')}</div>`;
}
